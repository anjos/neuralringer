...
bool lvl1::EMTrigger::filter (sys::Reporter& rep, const roiformat::RoI& roi) const
{
  ...
  RINGER_DEBUG1("Considering center at (eta,phi) = (" << roi.eta()
		<< "," << roi.phi() << ")"); 
  //are we, possibly at the wrap-around region for phi?
  bool wrap = false;
  if (roi.phi() > (roiformat::TWO_PI - roiformat::PI_THRESHOLD)) {
      wrap = true;
      RINGER_DEBUG3(roi.phi() << " is greater than " 
		    << (roiformat::TWO_PI - roiformat::PI_THRESHOLD));
      RINGER_DEBUG3("Possible Ring window at the phi wrap around"
		    << " region *DETECTED*.");
  }
  bool reverse_wrap = false;
  if (roi.phi() < roiformat::PI_THRESHOLD) {
      reverse_wrap = true;
      RINGER_DEBUG3(roi.phi() << " is smaller than " << roiformat::PI_THRESHOLD);
