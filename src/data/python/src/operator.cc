...
void bind_data_operator()
{
	...
	class_<data::NormalizationOperator, boost::shared_ptr<data::NormalizationOperator>, bases<data::PatternOperator>, boost::noncopyable>("LegacyNormalizationOperator", "Generates a Pattern with the same length as the previous, but with the mean removed and divided by the variance from each Pattern feature.", init<const data::Database&>((arg("pattern_set"))))
  ...
  class_<data::NormaliseOperator, boost::shared_ptr<data::NormaliseOperator>, bases<data::PatternOperator>, boost::noncopyable>("LegacyNormaliseOperator", "Generates a Pattern with the same length as the previous, but with the mean removed from each Pattern feature.", init<const data::Database&>((arg("pattern_set"))))
		;

  class_<data::RemoveDBMeanOperator, boost::shared_ptr<data::RemoveDBMeanOperator>, bases<data::PatternOperator>, boost::noncopyable>("LegacyRemoveDBMeanOperator", "Generates a Pattern with the same length as the previous, but with the mean removed from each Pattern feature.", init<const data::Database&>((arg("pattern_set"))))
		;
