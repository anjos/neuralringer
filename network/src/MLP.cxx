  ...
  : network::Network(reporter)
{
  ...
  for (size_t i=0; i<hidden.size(); ++i) {
    for (size_t j=0; j<hidden[i]; ++j) {
      network::Neuron* n = 
	new network::HiddenNeuron(neu_strat_type, neu_params);
      neurons.push_back(n);
      current.push_back(n);
    }
    layers.push_back(current);
    current.erase(current.begin(), current.end()); //clear 'current' container
    RINGER_DEBUG2("Hidden layer[" << i << "] (" 
		<< layers[layers.size()-1].size() <<" neurons) done.");
  }
  ...
  for (size_t i=0; i<output; ++i) {
    network::Neuron* n = 
      new network::OutputNeuron(neu_strat_type, neu_params);
    neurons.push_back(n);
    current.push_back(n);
  }
  layers.push_back(current);
  ...
  for (size_t i=0; i<layers.size()-1; ++i) {
	...
	network::Synapse* s = create_synapse(syn_strat_type, syn_params);
	RINGER_DEBUG3("Connect [" << i << "][" << j << "] -> [" << i+1
		    << "][" << k << "]");
  ...
  : network::Network(reporter)
{
  ...
  for (size_t i=0; i<hidden.size(); ++i) {
    for (size_t j=0; j<hidden[i]; ++j) {
      network::Neuron* n = 
	new network::HiddenNeuron(hidneu_strat_type, hidneu_params);
      neurons.push_back(n);
      current.push_back(n);
    }
    layers.push_back(current);
  ...
  for (size_t i=0; i<output; ++i) {
    network::Neuron* n = 
      new network::OutputNeuron(outneu_strat_type, outneu_params);
    neurons.push_back(n);
    current.push_back(n);
  }
  layers.push_back(current);
  ...
  for (size_t i=0; i<bias.size(); ++i) {
    if (!bneuron) bneuron = new network::BiasNeuron(1.0); //create only once
